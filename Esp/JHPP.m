...
+ (void)pushVC:(NSString *)vcString
      paramter:(NSDictionary *)dic
          from:(id)responder {
...
+ (void)pushVC:(NSString *)vcString
  ...
  UIViewController *topVC = [self fetchViewController:responder];
  UIViewController *vc =
      [JHUIViewControllerDecoupler jh_controllerFromString:vcString
                                                  paramter:dic];
  if (vc) {
    [topVC.navigationController pushViewController:vc animated:animated];
  }
...
+ (void)presentVC:(UIViewController *)vc
  ...
  UIViewController *topVC = [self fetchViewController:responder];
  if (navigation) {
    vc = [[[topVC.navigationController class] alloc]
        initWithRootViewController:vc];
  }
  [topVC presentViewController:vc animated:animated completion:nil];
}

+ (void)presentVC:(NSString *)vcString
         paramter:(NSDictionary *)dic
             from:(id)responder {
